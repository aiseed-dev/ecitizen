...
```jsonc
{
  ...
  "census": [...],            // JP: 21行×8列(1980-2015)。それ以外: 20行×9列(1980-2020, Ages2)
  "projection": [...],        // JP: 20行×7列(2015-2045)。それ以外: 20行×6列(2025-2050)
  "index": [...]              // JP: 15件(census 8+projection 7)。それ以外: 15件(census 9+projection 6)
}
```

- **JP のみ旧データのまま(変更なし)**: 旧 `App_Data/Population2015/Country/
  edJP.json`(census, 1980-2015) / `epJP.json`(projection, 2015-2045、
  平成30年推計)。
- **JP 以外 32 カ国(2026-07-05、Eurostat/ONS へ切替)**:
  ...
  - **census 最終年(2020)と projection 開始年(2025)は重複しない**
    ...
    非JPはその処理をしない。JP は旧データのまま重複あり、`[1:]` のまま)。
