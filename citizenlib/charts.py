...
# フォントはリポジトリに同梱しない。ビルド環境にインストールされた
# モリサワ BIZ UD ゴシックをレイアウト計算に使う (SVG は
# svg.fonttype='none' でテキスト出力し、閲覧側もシステムフォントで解決)。
# matplotlib はシステムフォント (~/.local/share/fonts 含む) を自動走査する。
_REQUIRED_FONT = "BIZ UDGothic"
if _REQUIRED_FONT not in {f.name for f in font_manager.fontManager.ttflist}:
    raise RuntimeError(
        f"チャート用フォント '{_REQUIRED_FONT}' が見つかりません。\n"
        "  Debian/Ubuntu: sudo apt install fonts-morisawa-bizud-gothic\n"
        "  sudo なし: Google Fonts の BIZ UDGothic の TTF を"
        " ~/.local/share/fonts/ に置く")
