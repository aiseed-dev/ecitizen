...
def _get(dataset: str, params: dict, geos: list) -> dict:
    query = urllib.parse.urlencode(params) + "".join(f"&geo={g}" for g in geos)
...
def fetch_census() -> None:
    """demo_pjangroup: 年ごとに32地域分(EU集計込み)を一括取得。"""
    geos = list(GEO_CODES.values())
    by_year = {}
    for year in CENSUS_YEARS:
        body = _get("demo_pjangroup", {"format": "JSON", "lang": "EN", "sex": "T", "time": year}, geos)
        by_year[year] = body
...
def fetch_projection() -> None:
    ...
    for year in COUNTRY_PROJECTION_YEARS:
        body = _get("proj_23np", {"format": "JSON", "lang": "EN", "sex": "T",
                                  "projection": "BSL", "time": year}, geos)
        by_year[year] = body
